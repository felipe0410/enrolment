...
class AttributeUpdateOptions
{
     ...
     * @throws LazyAssertionException
     */
    private static function validate(ParameterBag $params, array $externalLearningTypeAllow): array
    {
        $params = $params->all();

        $claim = Assert::lazy()->that($params, 'attributes')->notBlank();
        foreach ($params as $key => $value) {
            $claim
                ->that(EnrolmentAttributes::toNumeric($key), 'attribute');

            if (EnrolmentAttributes::S_URL == $key && !empty($value)) {
                $claim
                    ->that($value, 'url')->url();
            }
